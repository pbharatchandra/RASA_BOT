...
# load_dotenv()
# print(f"Gemini API Key present: {bool(os.getenv('GKEY'))}")
...
#         If unknown, say like, "I'm sorry, I don't have information on that topic." or similar and avoid writing codes even if the user is questioning you about writing code.
...
# KISHAN PIPLEINE BACKUP WITH POSTGRES LOGGING
...
# --- Google Sheets configuration (Optional Backup) ---
def get_google_sheets_client():
    try:
...
def log_fallback_to_postgres(user_message: str, intent_name: str = None, confidence: float = 0.0):
    """Logs fallback messages to PostgreSQL for auto-training"""
    conn = None
    try:
        ...
        query = """
            INSERT INTO fallback_logs (user_message, predicted_intent, confidence, is_trained)
            VALUES (%s, %s, %s, FALSE)
        """
        cursor.execute(query, (user_message, intent_name, confidence))
        conn.commit()
        print(f"✅ Logged fallback to Postgres: {user_message}")
    except Exception as e:
...
class ActionGenerativeFallback(Action):
    ...
    def run(self, dispatcher: CollectingDispatcher, tracker: Tracker, domain: Dict[Text, Any]) -> List[Dict[Text, Any]]:
        ...
        # 1. Log to Google Sheets (Backup)
        log_fallback_to_sheets(user_question, sender_id)

        # 2. Log to PostgreSQL (For Auto-Training)
        # We capture the low confidence intent if it exists, otherwise 'nlu_fallback'
        intent_info = tracker.latest_message.get('intent', {})
        predicted_intent = intent_info.get('name', 'nlu_fallback')
        confidence = intent_info.get('confidence', 0.0)
        
        log_fallback_to_postgres(user_message=user_question, intent_name=predicted_intent, confidence=confidence)
        
        # 3. Ask Gemini for a helpful response
        prompt = f"""You are a helpful assistant for NIST University. Answer user questions concisely.
        If unknown, say like, "I'm sorry, I don't have information on that topic." or similar. and handle if the users asking for writing
        User question: "{user_question}"
        """
        ...
        try:
            ...
            print(f"Error calling Gemini API: {e}")
            dispatcher.utter_message(response="utter_nlu_fallback")
            # log_fallback(user_question) # Using DB log instead
